const API_URL = "https://unnat-kisan.onrender.com";
...
async function loadPurchaseRequests() {
  ...
  try {
    const response = await fetch(`${API_URL}/api/farmer/purchase-requests`, {
      headers: {
...
async function acceptRequest(requestId) {
  const confirmed = confirm("Accept this purchase request?");

  if (!confirmed) return;

  await updateRequest(requestId, "accept");
}
...
async function rejectRequest(requestId) {
  const confirmed = confirm("Reject this purchase request?");

  if (!confirmed) return;

  await updateRequest(requestId, "reject");
}
...
async function loadMyCrops() {
  ...
  try {
    const response = await fetch(`${API_BASE}/api/farmer/crops`, {
      headers: {
        Authorization: `Bearer ${token}`,
      },
    });
